...
void Application::Start() {
    ...
    /* MOSS: Load WiFi from config if set */
    std::string moss_ssid = CONFIG_MOSS_WIFI_SSID;
    if (!moss_ssid.empty()) {
        auto& ssid_mgr = SsidManager::GetInstance();
        ssid_mgr.AddSsid(moss_ssid, CONFIG_MOSS_WIFI_PASSWORD);
        ESP_LOGI(TAG, "MOSS: WiFi credentials loaded (%s)", moss_ssid.c_str());
    }
    ...
    // MOSS: Skip OTA, use configured WebSocket URL
    {
        std::string ws_url = CONFIG_MOSS_WS_URL;
        Settings settings("websocket", true);
        settings.SetString("url", ws_url);
        ESP_LOGI(TAG, "MOSS: WebSocket URL: %s", ws_url.c_str());

        // Initialize the protocol directly
    ...
    if (protocol_started) {
        std::string message = std::string(Lang::Strings::VERSION) + "MOSS-local";
